...
def get_anggaran_manager():
    print(">>> STREAMLIT: Menginisialisasi Anggaran Harian...")
...
def halaman_riwayat(anggaran: AnggaranHarian):
    ...
    with st.spinner("Memuat riwayat..."):
        ...
        if df_transaksi is None:
            ...
            st.write("### 🗑️ Menu Hapus Transaksi")
            
            with st.form("form_hapus"):
                id_yang_dipilih = st.number_input("Masukkan ID Transaksi yang ingin dihapus:", min_value=1, step=1, value=1)
                tombol_hapus = st.form_submit_button("Hapus Transaksi Terpilih")
                
                if tombol_hapus:
                    st.session_state.pemicu_hapus = id_yang_dipilih

            if "pemicu_hapus" in st.session_state and st.session_state.pemicu_hapus == id_yang_dipilih:
                st.warning(f"Apakah Anda yakin ingin menghapus Transaksi dengan ID {id_yang_dipilih}?")
                
                col_konfirm1, col_konfirm2 = st.columns([1, 10])
                with col_konfirm1:
                    if st.button("Ya, Hapus", type="primary"):
                        with st.spinner("Menghapus..."):
                            if anggaran.hapus_transaksi(id_yang_dipilih):
                                st.toast(f"Transaksi ID {id_yang_dipilih} berhasil dihapus!", icon="🗑️")
                                st.cache_data.clear()
                                del st.session_state.pemicu_hapus
                                st.rerun()
                            else:
